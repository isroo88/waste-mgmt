...
function Billing() {
  ...
  const [printCollectedBy, setPrintCollectedBy] = useState(null);
  ...
  async function handleSubmit(e) {
    ...
    if (!selectedCustomer || !bill.receipt_number || !bill.from_month || !bill.to_month) {
      setError('Please select a customer and fill all required fields.'); return;
    }
    setSubmitting(true);
    ...
    // Show print preview
    setPrintBill({ ...billData, monthly_fee: selectedCustomer.monthly_fee });
    setPrintCustomer(selectedCustomer);
    setPrintCollectedBy(staffData);
  ...
  // Open print preview for existing bill
  async function openPrint(b) {
    ...
    const { data: staff } = await supabase.from('app_users')
      .select('full_name, staff_code').eq('id', b.generated_by).single();
    setPrintBill(b);
    setPrintCustomer(cust);
    setPrintCollectedBy(staff);
  }
    ...
    <Layout title="Billing">
      {printBill && (
        <PrintBill
          ...
          collectedBy={printCollectedBy}
          settings={settings}
          onClose={() => { setPrintBill(null); setPrintCustomer(null); setPrintCollectedBy(null); }}
        />
      )}
        ...
        <form style={s.form} onSubmit={handleSubmit}>
          ...
          <div style={s.field}>
            ...
            {selectedCustomer ? (
              ...
              <>
                <input style={s.input} placeholder="Search by name, phone or customer code..." value={search}
          ...
          <div style={s.grid3}>
            <div><label style={s.label}>From Month *</label><input style={s.input} value={bill.from_month} onChange={(e) => setBill({ ...bill, from_month: e.target.value })} placeholder="e.g. Baisakh 2083" required /></div>
            <div><label style={s.label}>To Month *</label><input style={s.input} value={bill.to_month} onChange={(e) => setBill({ ...bill, to_month: e.target.value })} placeholder="e.g. Ashadh 2083" required /></div>
            <div><label style={s.label}>Months</label>
          ...
          <div style={s.grid2}>
            <div><label style={s.label}>Previous Dues (Rs.)</label><input style={s.input} type="number" min="0" value={bill.previous_dues} onChange={(e) => setBill({ ...bill, previous_dues: e.target.value })} /></div>
            <div><label style={s.label}>Penalty / Fine (Rs.)</label><input style={s.input} type="number" min="0" value={bill.penalty} onChange={(e) => setBill({ ...bill, penalty: e.target.value })} /></div>
          </div>
              ...
              {recentBills.length === 0
                ...
                : recentBills.map((b) => (
                  <tr key={b.id}>
                    ...
                    <td style={s.td}><button style={s.printBtn} onClick={() => openPrint(b)}>🖨 Print</button></td>
                  </tr>
                ))
              }
            </tbody>
