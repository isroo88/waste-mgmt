...
import { toBS, toBSMonth, bsToAD, monthsOverdueFromYM } from '../../lib/dateUtils';
...
function calcToMonth(fromDateBS, numMonths) {
  ...
  const { getMonthName } = require('../../components/BSMonthPicker');
...
function CustomerDetail() {
  ...
  async function handleToggleStatus() {
    ...
    if (newStatus === 'deactivated') {
      router.push(customer.customer_type === 'business' ? '/businesses' : '/customers');
    } else {
      loadData();
    }
  }
  ...
  async function handleGenerateBill(e) {
    ...
    const fromLabel = yyyymmToLabel(bill.from_ym);
    const toLabel = yyyymmToLabel(bill.to_ym);
    ...
    const { data: billData, error: billErr } = await supabase.from('bills').insert({
      ...
      to_month: bill.to_ym,
      period_label: `${fromLabel} – ${toLabel}`,
    ...
    setPrintData({ bill: { ...billData, monthly_fee: customer.monthly_fee, from_month: bill.from_ym, to_month: bill.to_ym }, customer, collectedBy: staffData });
  ...
  const s = {
    ...
    deactivatedBanner: { background: '#fee2e2', border: '1px solid #fecaca', borderRadius: 10, padding: '12px 16px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
    ...
    dangerBtn: { padding: '10px 20px', borderRadius: 8, border: '1px solid #fecaca', background: '#fff', color: '#b91c1c', fontWeight: 600, fontSize: 14, cursor: 'pointer' },
    ...
    confirmBox: { background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 10, padding: 20, marginTop: 20 },
  };
    ...
    <Layout title={customer.name}>
        ...
        <div style={s.overlay} onClick={(e) => { if (e.target === e.currentTarget) setShowBillModal(false); }}>
          <div style={s.modal}>
            ...
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13 }}>
              ...
              <br />Fee: <strong>Rs. {Number(customer.monthly_fee).toLocaleString()}/month</strong>
      ...
      {isDeactivated && (
        <div style={s.deactivatedBanner}>
          <div>
            <strong style={{ color: '#b91c1c' }}>⚠ This customer is deactivated</strong>
            <p style={{ margin: '2px 0 0', fontSize: 13, color: '#b91c1c' }}>They are hidden from active lists and billing. Records are preserved.</p>
          </div>
          <button style={s.btn('#22c55e')} onClick={handleToggleStatus} disabled={deactivating}>
      ...
      <div style={s.header}>
        <div>
          <h2 style={{ ...s.name, opacity: isDeactivated ? 0.6 : 1 }}>{customer.name}</h2>
      ...
      <div style={s.grid3}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e2e8f0' }}>
          ...
          <p style={{ ...s.infoValue, color: '#15803d', fontSize: 17 }}>{paidUpToYM ? yyyymmToLabel(paidUpToYM) : '—'}</p>
        </div>
        <div style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e2e8f0' }}>
          <p style={s.infoLabel}>Last Payment</p>
          <p style={{ ...s.infoValue, fontSize: 17 }}>{lastPaymentDateYM ? yyyymmToLabel(lastPaymentDateYM) : '—'}</p>
        </div>
        <div style={{ background: overdueBg, borderRadius: 12, padding: 16, border: '1px solid #e2e8f0' }}>
          <p style={{ ...s.infoLabel, color: overdueColor }}>{overdue === null ? 'Status' : overdue <= 0 ? 'Status' : 'Months Overdue'}</p>
          <p style={{ ...s.infoValue, color: overdueColor, fontSize: 17 }}>
            {overdue === null ? 'No bills yet' : overdue <= 0 ? '✓ Up to date' : `${overdue} month${overdue !== 1 ? 's' : ''}`}
          </p>
        </div>
      </div>
      ...
      {tab === 'profile' && (
        <div style={s.section}>
          <div style={s.grid}>
            ...
            {customer.pan_number && <div><p style={s.infoLabel}>PAN Number</p><p style={s.infoValue}>{customer.pan_number}</p></div>}
          </div>
          ...
          {!isDeactivated ? (
            !showDeactivateConfirm ? (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <strong style={{ fontSize: 14, color: '#b91c1c' }}>Deactivate Customer</strong>
                  <p style={{ fontSize: 13, color: '#64748b', margin: '2px 0 0' }}>Hide from active lists. All records are kept.</p>
                </div>
                <button style={s.dangerBtn} onClick={() => setShowDeactivateConfirm(true)}>Deactivate</button>
              </div>
            ) : (
              <div style={s.confirmBox}>
                <strong style={{ fontSize: 14, color: '#92400e' }}>⚠ Confirm Deactivation</strong>
                <p style={{ fontSize: 13, color: '#92400e', margin: '6px 0 16px' }}>
                  <strong>{customer.name}</strong> will be hidden from the active customer list and billing. Their payment records will be preserved. You can reactivate them anytime.
                </p>
                <div style={{ display: 'flex', gap: 10 }}>
                  <button style={s.btn('#b91c1c')} onClick={handleToggleStatus} disabled={deactivating}>
                    {deactivating ? 'Deactivating...' : 'Yes, Deactivate'}
                  </button>
                  <button style={s.outlineBtn} onClick={() => setShowDeactivateConfirm(false)}>Cancel</button>
                </div>
              </div>
            )
          ) : null}
